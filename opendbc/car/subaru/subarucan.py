...
def create_es_distance(packer, frame, es_distance_msg, bus, pcm_cancel_cmd, long_enabled = False, brake_cmd = False, cruise_throttle = 0,
  ...
  # Stands in for a press of the resume button on the wheel, which is what the camera itself
  # puts on this bit. Cancel wins if both are somehow asked for, and the panda refuses the
  # pair outright, so the two can never leave here together.
  if cruise_resume_cmd and not pcm_cancel_cmd:
